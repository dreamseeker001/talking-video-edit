...
本版完善偏好合同、全片剪辑计划及 Agent 兜底与交付汇报。脚本负责确定性阶段，Agent 负责语义判断和授权范围内的异常恢复。现有辅助程序测试已通过；新版编排仍待完整新片端到端实测。

[English](README.md) | **简体中文**

参见[更新日志](CHANGELOG.zh-CN.md)了解版本变更。
...
长期偏好不是散落的记忆文本，而是一份带版本、场景范围、模块/adapter 映射、可执行设置、策略证据和实现验证状态的配置合同。新任务冻结合同快照；预检与渲染读取同一快照。明确说“记住这个”会启动带备份和并发检查的更新，写入后完整复读并检查冲突。
